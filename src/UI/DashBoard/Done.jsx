...
const Done = ({id,taskUI,setTaskUI}) => {
    ...
    const doneTasks = taskUI?.filter(t => t.status === 'done') || [];//array of doneTasks only
...
const handleTask = (e)=>{
          ...
          .catch((error)=>{
            ...
            console.log("Couldn't submitted Done task to database & errorCode is : ",errorCode);
            const errorMessage = error.message;
            console.log("Couldn't submitted Done task to database & errorMessage is : ",errorMessage);
  ...
  return (
    <div>
        ...
        <div onMouseLeave={()=>setHeading(false)} className="flex">
            <div className="bg-blue-400 rounded-md pl-1.5 w-23 mt-2 ml-2.5 md:ml-2 md:mt-4 md:w-78 md:text-2xl">DONE</div>
        ...
        <div onMouseEnter={()=>setHeading(true)} className="flex">
            <div className="mt-2 ml-2.5 pl-1.5 rounded-md w-23 md:text-2xl md:ml-2 md:mt-4 md:w-78">DONE</div>
        </div>
                        ...
                        absolute z-10 top-6 right-10 md:w-34 md:h-44 md:top-7 md:right-28 bg-black">
                          {
                            ...
                            <>
                             ...
                             <div className="absolute w-14 left-14 md:w-34 md:left-32 flex flex-col items-center rounded-md border border-[#3b82f6] cursor-pointer bg-black">
                                <div onClick={()=>handleStatusChange(t._id,"todo")} className="w-full text-center font-semibold rounded-tl-sm rounded-tr-sm border-b-2 border-b-[#3b82f6] hover:bg-indigo-400">To Do</div>
                                <div onClick={()=>handleStatusChange(t._id,"in-progress")} className="w-full text-center font-semibold rounded-bl-sm rounded-br-sm hover:bg-indigo-400">In Progress</div>
                             </div>
                            </> 
                            : 
                            <>
